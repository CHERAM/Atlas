...
class CleanupTests(unittest.TestCase):
    ...
    def _create_dirs(self, root: Path) -> tuple[Path, Path, Path, Path]:
        cache_dir = root / ".atlas-cache"
        github_dir = root / ".github" / "atlas"
        books_dir = root / ".atlas" / "books"
        cache_dir.mkdir(parents=True, exist_ok=True)
        github_dir.mkdir(parents=True, exist_ok=True)
        books_dir.mkdir(parents=True, exist_ok=True)
        (cache_dir / "dummy.txt").write_text("x", encoding="utf-8")
        (github_dir / "context.md").write_text("x", encoding="utf-8")
        (books_dir / "Atlas-Prompt-Creation.md").write_text("x", encoding="utf-8")
        return cache_dir, github_dir, books_dir, root / "atlas.yaml"

    def test_remove_cache_only(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_config(root)
            cache_dir, github_dir, books_dir, config_path = self._create_dirs(root)
            ...
            self.assertTrue(books_dir.exists())
